      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 p-6 bg-gray-50">
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            ...
            <p className="text-2xl font-semibold">{loading ? "--" : data?.pagination?.total || 0}</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border">
            <h3 className="text-sm font-medium text-gray-500 mb-1">
              {status === "payout" ? "Total Payouts" : "Total Referrals"}
            </h3>
            <p className="text-2xl font-semibold">{loading ? "--" : displayTotals.referrals || 0}</p>
          </div>
          {status !== "payout" && (
            <>
              <div className="bg-white p-4 rounded-lg shadow-sm border">
                <h3 className="text-sm font-medium text-gray-500 mb-1">Pending</h3>
                <p className="text-2xl font-semibold text-yellow-600">{loading ? "--" : displayTotals.pending || 0}</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm border">
                <h3 className="text-sm font-medium text-gray-500 mb-1">Completed</h3>
                <p className="text-2xl font-semibold text-green-600">{loading ? "--" : displayTotals.completed || 0}</p>
              </div>
            </>
          )}
          {status === "payout" && (
            <div className="bg-white p-4 rounded-lg shadow-sm border">
              ...
              <p className="text-2xl font-semibold text-yellow-600">{loading ? "--" : displayTotals.payout || 0}</p>
            </div>
          )}
          <div className="bg-white p-4 rounded-lg shadow-sm border">
        ...
        <div className="px-6 py-4 border-b bg-gray-50">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex items-end">
              <button onClick={clearFilters} className="w-full p-2 bg-gray-200 hover:bg-gray-300 rounded-md">
                Clear Filters
              </button>
            </div>
          </div>
              ...
              {loading ? (
                ...
                allReferrals.map((ref, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{(page - 1) * 10 + index + 1}</td>
        ...
        <div className="px-6 py-4 bg-white border-t flex justify-between items-center">
          <div className="text-sm text-gray-600">
            {data?.pagination?.total > 0 && `Total Amount: ₦${calculateTotalAmount().toLocaleString()}`}
          </div>
